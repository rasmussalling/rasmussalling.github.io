...
do [t = i * (i + 1) / 2] 
  i != n ->
    ...
    {t = i * (i + 1) / 2}
od
{i = n & t = n * (n + 1) / 2}`;var Ne=ce('<svg><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12.75L11.25 15L15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.75 3.75 0 0 1-1.043 3.296a3.75 3.75 0 0 1-3.296 1.043A3.75 3.75 0 0 1 12 21c-1.268 0-2.39-.63-3.068-1.593a3.75 3.75 0 0 1-3.296-1.043a3.75 3.75 0 0 1-1.043-3.296A3.75 3.75 0 0 1 3 12c0-1.268.63-2.39 1.593-3.068a3.75 3.75 0 0 1 1.043-3.296a3.75 3.75 0 0 1 3.296-1.043A3.75 3.75 0 0 1 12 3c1.268 0 2.39.63 3.068 1.593a3.75 3.75 0 0 1 3.296 1.043a3.75 3.75 0 0 1 1.043 3.296A3.75 3.75 0 0 1 21 12"></path></svg>');function Fe(e,s){const d=Ee(s,["$$slots","$$events","$$legacy"]);var l=Ne();let i;C(()=>i=Oe(l,i,{viewBox:"0 0 24 24",width:"1.2em",height:"1.2em",...d})),b(e,l)}const qe=(e,s,d,l)=>{if(t(s)){a(d,null);try{a(l,t(s)(),!0)}catch(i){a(d,i instanceof Error?i.message:"Failed to generate challenge",!0)}}},Ge=(e,s,d,l)=>{if(t(s)){a(d,null);try{a(l,t(s)(),!0)}catch(i){a(d,i instanceof Error?i.message:"Failed to generate sample program",!0)}}};var He=k('<meta name="description" content="Chip">'),Je=k("The program is <b>fully annotated</b>",1),We=k("The program is <b><i>not</i> fully annotated</b>",1),Ye=k('<span class="ml-4 text-base text-white/80"> </span>'),Ze=k("<option> </option>"),ze=k('<!> <div class="relative grid grid-rows-[2fr_auto_auto] overflow-hidden bg-slate-800"><!> <div><span class="font-bold"> </span> <div class="flex-1"></div> <span class="text-xl"><!></span> <!> <button class="ml-4 rounded bg-slate-900/60 px-3 py-1 text-lg transition hover:bg-slate-900 disabled:opacity-60">Example</button> <button class="ml-4 rounded bg-slate-900/60 px-3 py-1 text-lg transition hover:bg-slate-900 disabled:opacity-60">Challenge</button> <select class="ml-4 rounded bg-slate-900/60 px-3 py-1 text-lg transition hover:bg-slate-900"><option disabled selected>Loops</option><!></select></div></div>',1);function st(e,s){me(s,!0);const d=`{ true }
if
  false -> skip
fi
{ true }`,l="chip-program";let i=g(E((typeof localStorage.getItem(l)=="string"?localStorage.getItem(l):null)||d));F(()=>{localStorage.setItem(l,t(i))});let x=g(E({parse_error:!1,prelude:"",assertions:[],markers:[],is_fully_annotated:!1})),c=g(E([])),f=g(!1),p=g("idle"),O=g(null),A=g(null),P=g(null),R=g(null);const Q=Object.values(Object.assign({"./exercises/exercise1.txt":Pe,"./exercises/exercise1s.txt":Re,"./exercises/exercise2.txt":De,"./exercises/exercise2s.txt":Le,"./exercises/exercise3.txt":Me,"./exercises/exercise3s.txt":$e,"./exercises/exercise4.txt":Ve,"./exercises/exercise4s.txt":je}));let H=E(Q.map((r,n)=>{const o=r.split(`
`),u=o.find(_=>_.startsWith("// Algorithm:")),v=o.find(_=>_.startsWith("// Solution:"));return{label:u?u.replace("// Algorithm:","").trim():v?v.replace("// Solution:","S:").trim():`Exercise ${n+1}`,content:r}}));const X=r=>{a(i,r,!0)};pe(()=>{(async()=>{const n=await B(()=>import("../chunks/Doc3GtEQ.js"),[],import.meta.url);await n.default(),a(O,n.parse,!0),a(P,n.generateSampleProgram,!0),a(R,n.generateChallenge,!0)})().catch(console.error)}),F(()=>{if(!t(O))return;a(f,!1);const r=t(O)(t(i));r.parse_error&&a(f,!0),a(x,r,!0)});let D=0;F(()=>{const r=Te(t(x));let n=()=>{};return(async()=>{const u=++D,v=await B(()=>import("../chunks/BQp3cmfl.js"),__vite__mapDeps([0,1,2]),import.meta.url);a(c,[],!0),a(p,"verifying");let m=!1,_=!1;for(const y of r.assertions){const{cancel:se,result:oe}=v.run(y.smt,{prelude:r.prelude});n=se;const S=await oe;if(S=="cancelled")return;_||(_=S[S.length-1].trim()==="timeout");const le=S[S.length-1].trim()==="unsat";if(u!==D){console.info("aborted",u,D,r,S);return}(!le||_)&&(m=!0,a(c,[...ve(()=>t(c)),{severity:"Error",tags:[],message:y.text?y.text:"Verification failed",span:y.span,relatedInformation:[]},...y.related?[{severity:"Info",tags:[],message:y.related[0],span:y.related[1],relatedInformation:[]}]:[]],!0))}m?a(p,_?"timeout":"error",!0):a(p,"verified")})().catch(console.error),()=>{n()}});var J=ze();ye(r=>{var n=He();he.title="Chip",b(r,n)});var W=K(J);Ae(W,{title:"Chip",Icon:Fe,onProgramChange:X});var Y=h(W,2),Z=w(Y);const ee=ge(()=>[...t(x).markers,...t(c)]);ke(Z,{get markers(){return t(ee)},get value(){return t(i)},set value(r){a(i,r,!0)}});var L=h(Z,2),M=w(L),te=w(M,!0);I(M);var $=h(M,4),re=w($);{var ne=r=>{var n=de(),o=K(n);{var u=m=>{var _=Je();U(),b(m,_)},v=m=>{var _=We();U(),b(m,_)};G(o,m=>{t(x).is_fully_annotated?m(u):m(v,!1)})}b(r,n)};G(re,r=>{!t(f)&&t(p)=="verified"&&r(ne)})}I($);var z=h($,2);{var ie=r=>{var n=Ye(),o=w(n,!0);I(n),C(()=>q(o,t(A))),b(r,n)};G(z,r=>{t(A)&&r(ie)})}var V=h(z,2);V.__click=[Ge,P,A,i];var j=h(V,2);j.__click=[qe,R,A,i];var N=h(j,2);N.__change=r=>{const n=r.target,o=H.find(u=>u.label===n.value);o&&a(i,o.content,!0),n.selectedIndex=0};var ae=h(w(N));we(ae,17,()=>H,Ie,(r,n)=>{var o=Ze(),u={},v=w(o,!0);I(o),C(()=>{u!==(u=t(n).label)&&(o.value=(o.__value=t(n).label)==null?"":t(n).label),q(v,t(n).label)}),b(r,o)}),I(N),I(L),I(Y),C(()=>{Se(L,1,`flex items-center p-2 text-2xl text-white transition duration-500 ${(t(f)?"bg-purple-600":t(p)==="verified"&&!t(x).is_fully_annotated?"bg-emerald-400":{idle:"bg-gray-500",verifying:"bg-yellow-500",verified:"bg-green-500",error:"bg-red-500",timeout:"bg-blue-500"}[t(p)])??""}`),q(te,t(f)?"Parse error":{idle:"Idle",verifying:"Verifying...",verified:"Verified",error:"Verification error",timeout:"Verification timed out"}[t(p)]),V.disabled=!t(P),j.disabled=!t(R)}),b(e,J),xe()}be(["click","change"]);export{st as component};
